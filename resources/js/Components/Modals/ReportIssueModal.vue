...
<template>
  <transition name="fade">
    <div v-if="show" @click.self="emit('close')" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="relative bg-white rounded-lg shadow-lg w-full max-w-3xl overflow-y-auto max-h-screen p-4">
        <!-- ❌ Close Button -->
        ...
        <!-- 🔙 Back Link -->
        <!-- <div class="mt-6 px-4">
          <a href="/dashboard" class="inline-flex items-center text-blue-600 hover:underline text-sm font-medium transition">
            ← Back to Dashboard
          </a>
        </div> -->
    ...
    <form @submit.prevent="submit" class="bg-white p-6 rounded-b-lg shadow space-y-6">
      ...
      <div>
        ...
        <select name="type"  v-model="form.problem_type" required
    ...
    <option value="Murky Water">Murky Water</option>
