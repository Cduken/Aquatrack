...
const startCameraPreview = async () => {
  try {
    ...
    Swal.fire({ icon: 'error', title: 'Camera permission denied', text: 'Allow access to use the camera.' })
  }
}
...
const capturePhoto = async () => {
  ...
  if (blob && blob.size <= MAX_FILE_SIZE) {
    ...
    Swal.fire({ icon: 'error', title: 'Image is too large or failed to capture' })
  }
}
...
const submit = () => {
  ...
  stopCamera() // close stream before submitting
...
<!-- Camera Capture Mode (Preview & Capture) -->
<div v-else>
  <div class="rounded-md overflow-hidden border shadow mb-2">
    <video ref="videoRef" autoplay playsinline muted class="w-full h-64 object-cover bg-black" />
  </div>
  <div class="flex justify-center gap-4 mb-2">
    <button @click="startCameraPreview"
      class="px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 font-medium">
      Start Camera
    </button>
  ...
  <p class="text-xs text-gray-500 text-center">
    Photo will be captured at 720p. Limit 10MB. Camera permission is required.
  </p>
</div>
