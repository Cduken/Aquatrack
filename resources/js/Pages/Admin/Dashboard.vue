...
const canViewActivityLog = page.props.auth?.user?.can?.('view-activity-log') ?? false;
...
<template>
    <AdminLayout title="Dashboard">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden h-full mb-4">
            <div
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden h-full">
                ...
                <div class="divide-y divide-gray-100">
                    <div v-for="activity in recentActivities" :key="activity.id"
                        ...
                        <div :class="`p-2 rounded-full ${getActivityBgColor(activity)}`">
                            <svg v-if="getActivityIcon(activity) === 'plus-circle'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <svg v-else-if="getActivityIcon(activity) === 'pencil-square'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'trash'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'arrow-right-on-rectangle'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'arrow-left-on-rectangle'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'shield-check'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'shield-exclamation'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'sparkles'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'flag'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z">
                            ...
                            <svg v-else-if="getActivityIcon(activity) === 'arrow-path'" class="w-5 h-5"
                                :class="`text-${getActivityTextColor(activity)}`" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-800 truncate">{{ getActivityTitle(activity) }}</p>
                            <p class="text-xs text-gray-500 mt-1 truncate">
                                {{ activity.description }}
                                <span v-if="activity.properties?.tracking_code" class="font-medium">
                                    ({{ activity.properties.tracking_code }})
                                </span>
                            </p>
                            <p class="text-xs text-gray-400 mt-1 truncate">
                                <span v-if="activity.causer_name">By {{ activity.causer_name }}</span>
                                <span v-else>By System</span>
                                <span v-if="activity.properties?.old_status" class="ml-2">
                                    Status: {{ activity.properties.old_status }} → {{ activity.properties.new_status }}
                                </span>
                            </p>
                        </div>
                    ...
                    <div v-if="recentActivities.length === 0" class="p-6 text-center text-gray-500">
